...
import "../interfaces/IVaporCrowdsale.sol";
import "../interfaces/ISushiToken.sol";
import "../interfaces/IVaporLauncher.sol";

// MVP for preparing a Vapor set menu
...
contract VaporRecipe01 {

    using SafeMath for uint256;

    IVaporTokenFactory public tokenFactory;
    IVaporMarket public VaporMarket;
    IWETH public weth;
    IVaporLauncher public VaporLauncher; 
    IVaporFarmFactory public farmFactory;
     ...
     * @param _VaporMarket - Factory that produces a market / auction to sell your tokens
     * @param _VaporLauncher - VaporLauncher is a vault that collects tokens and sends them to SushiSwap
    ...
    constructor(
        ...
        address _weth,
        address _VaporMarket,
        address _VaporLauncher,
        address _uniswapFactory,
        address _farmFactory
    ) {
        tokenFactory = IVaporTokenFactory(_tokenFactory);
        weth = IWETH(_weth);
        VaporMarket = IVaporMarket(_VaporMarket);
        VaporLauncher = IVaporLauncher(_VaporLauncher);
        uniswapFactory = _uniswapFactory;
        farmFactory = IVaporFarmFactory(_farmFactory);

    }

    /** 
     * @dev Gateway to prepare a Vapor recipe
     *   
    */
    function prepareVapor(
        ...
        external payable
    {
        ...
        token.approve(address(VaporMarket), tokensToMarket);

        // Scope for adding liquidity
        uint256 templateId = VaporLauncher.currentTemplateId(1);
        IPoolLiquidity poolLiquidity = IPoolLiquidity(VaporLauncher.createLauncher(templateId,address(token),0,payable(msg.sender),""));
        ...
        IVaporCrowdsale crowdsale = IVaporCrowdsale(VaporMarket.createCrowdsale(
            address(token), 
            tokensToMarket, 
