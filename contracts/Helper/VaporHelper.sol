...
contract BaseHelper {
    IVaporMarketFactory public market;
    ...
    function setContracts(
        ...
        require(
            ...
            "VaporHelper: Sender must be Admin"
        );
        if (_market != address(0)) {
            market = IVaporMarketFactory(_market);
        }
...
interface ICrowdsale is IVaporMarket {
...
interface IDutchAuction is IVaporMarket {
    function marketStatus() external view returns(
        uint128 commitmentsTotal,
        bool finalized,
        bool usePointList
...
interface IBatchAuction is IVaporMarket {
    function marketStatus() external view returns(
...
interface IHyperbolicAuction is IVaporMarket {
    function marketStatus() external view returns(
        uint128 commitmentsTotal,
        bool finalized,
        bool usePointList
...
contract MarketHelper is BaseHelper, TokenHelper, DocumentHepler {
    ...
    function getUserMarketInfo(address _action, address _user) public view returns(UserMarketInfo memory userInfo) {
        IVaporMarket market = IVaporMarket(_action);
...
contract VaporHelper is MarketHelper, FarmHelper {
