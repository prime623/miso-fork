...
contract VaporTokenFactory is CloneFactory, SafeTransfer{
    ...
    /// @notice Event emitted when first initializing Vapor Token Factory.
    event VaporInitTokenFactory(address sender);
    ...
    /**
     * @notice Single gateway to initialize the Vapor Token Factory with proper address.
     * @dev Can only be initialized once.
     * @param _accessControls Sets address to get the access controls from.
     */
    /// @dev GP: Migrate to the BentoBox.
    function initVaporTokenFactory(address _accessControls) external  {
        require(!initialised);
        initialised = true;
        locked = true;
        accessControls = VaporAccessControls(_accessControls);
        emit VaporInitTokenFactory(msg.sender);
    }
    ...
    function setMinimumFee(uint256 _amount) external {
        require(
            ...
            "VaporTokenFactory: Sender must be operator"
        );
        minimumFee = _amount;
    }
    ...
    function setIntegratorFeePct(uint256 _amount) external {
        ...
        require(
            _amount <= 1000, 
            "VaporTokenFactory: Range is from 0 to 1000"
        );
    ...
    function setLocked(bool _locked) external {
        require(
            accessControls.hasAdminRole(msg.sender),
            "VaporTokenFactory: Sender must be admin"
        );
        locked = _locked;
    }
        ...
        public payable returns (address token)
    {
        ...
        if (locked) {
            require(accessControls.hasAdminRole(msg.sender) 
                ...
                "VaporTokenFactory: Sender must be minter if locked"
            );
        }
        require(msg.value >= minimumFee, "VaporTokenFactory: Failed to transfer minimumFee");
        ...
        console.log("Clone address: ", address(token));
        ...
        external payable returns (address token)
    {
        ...
        IVaporToken(token).initToken(_data);
    ...
    function addTokenTemplate(address _template) external {
        ...
        uint256 templateType = IVaporToken(_template).tokenTemplate();
