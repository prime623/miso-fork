...
contract VaporAccessFactory is CloneFactory {
    ...
    /// @notice Event emitted when first initializing Vapor AccessControl Factory.
    event VaporInitAccessFactory(address sender);
    ...
    /**
     * @notice Single gateway to initialize the Vapor AccessControl Factory with proper address and set minimum fee.
     * @dev Can only be initialized once.
     * @param _minimumFee Minimum fee number.
     * @param _accessControls Address of the access controls.
     */
    function initVaporAccessFactory(uint256 _minimumFee, address _accessControls) external {
        require(!initialised);
        initialised = true;
        minimumFee = _minimumFee;
        accessControls = VaporAccessControls(_accessControls);
        emit VaporInitAccessFactory(msg.sender);
    }
    ...
    function deployAccessControl(address _admin) external payable returns (address access) {
        ...
        VaporAccessControls(access).initAccessControls(_admin);
    ...
    function updateAccessTemplate(address _template) external {
        require(
            ...
            "VaporAccessFactory.updateAccessTemplate: Sender must be admin"
        );
    ...
    function setDev(address _devaddr) external {
        require(
            accessControls.hasAdminRole(msg.sender),
            "VaporAccessFactory.setMinimumFee: Sender must be admin"
        );
